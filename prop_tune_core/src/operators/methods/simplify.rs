        ...
        match self.normal() {
            ...
            Proposition::Composition(comp) => match *comp {
                ...
                Operator::Or(a, b) => {
                    ...
                    let props: Vec<Proposition> = props
                        .into_iter()
    ...
    fn gather_propositions(self, props: &mut std::collections::HashSet<Proposition>) {
        match self {
            Proposition::Condition(Condition::True) => {
                props.insert(Proposition::new_true());
            }
            Proposition::Predicate(pred) => {
                props.insert(Proposition::new_pred(pred));
            }
            Proposition::Composition(cond) => match *cond {
                Operator::And(a, b) => {
                    props.insert(Proposition::new_and(a, b).simplify());
                }
                Operator::Or(a, b) => {
                    a.gather_propositions(props);
                    b.gather_propositions(props)
                }
                Operator::Not(pred) => {
                    props.insert(Proposition::new_not(pred));
                }
                _ => (),
            },
            _ => (),
        ...
        P: Into<Proposition> + Clone,
    {
        ...
        match n.cmp(&1) {
            Ordering::Less => wrap(
                props
                    .get(0)
                    .expect("there to be a proposition")
                    .clone()
                    .into(),
            ),
            _ => props
                .iter()
                .take(n)
                .fold(wrap(props.index(n).clone().into()), |prev, prop| {
                    comp(prev, wrap(prop.clone().into()))
                }),
        }
    }
    ...
    fn account_predicates(
        ...
        match self {
            ...
            Proposition::Composition(comp) => match comp.as_ref() {
                ...
                Operator::And(a, b) => {
                    a.account_predicates(preds, nots) && b.account_predicates(preds, nots)
                }
                _ => unreachable!(),
            },
        }
...
mod test_simplify {
    ...
    fn test_simplify() {
        let cases = vec![(
            Proposition::new_not(Proposition::new_or("A", Proposition::new_and("B", "C"))),
            Proposition::new_or(
                Proposition::new_and(Proposition::new_not("A"), Proposition::new_not("B")),
                Proposition::new_and(Proposition::new_not("A"), Proposition::new_not("C")),
            ),
        )];

        cases.into_iter().for_each(|(mut input, expected)| {
            input = input.demorg();
            input = input.normal();
            input = input.simplify();
            assert_eq!(expected, input.simplify());
        })
    }
}
